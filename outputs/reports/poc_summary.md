...
## HIGH 32건을 육안 검수한 결과 (중요)

HIGH 후보 32건의 T1/T2 이미지 chip을 실제로 하나씩 열어봤다. 10m 해상도라
개별 픽셀 블록을 "이건 확실히 건물이다"라고 단정하긴 어려웠지만, 훨씬 더
중요한 패턴을 발견했다:

```text
32건의 HIGH 건물 -> 실제로 겹치는 change_polygon(현장)은 11곳뿐
  - CHG_00002 하나가 건물 11개에 걸침
  - CHG_00027 하나가 건물 10개에 걸침
  (이 둘이 HIGH 32건 중 21건, 66%를 차지)
```

즉 **"HIGH 32건"은 서로 다른 32곳이 아니라, 대형 변화구역(아마 대단지
아파트 공사장) 몇 곳이 여러 건물 footprint에 걸쳐서 여러 번 집계된
것**이다. 실제 현장조사팀이 방문해야 할 곳은 32곳이 아니라 11곳에
가깝다. 이 구분을 위해 `overlay.py`에 `site_id`(어느 change_polygon에
속하는지) 컬럼을 추가했다 - `building_change_results.gpkg`에서
`site_id`로 group-by하면 실제 현장 수를 바로 셀 수 있다.

전체 76건 기준으로도 서로 다른 현장은 33곳이다(건물 수 기준 76 대비
57% 감소). **현장조사 우선순위 리포트를 사람에게 줄 때는 반드시 건물
개수가 아니라 site_id 기준 현장 수로 안내해야 한다** - 그렇지 않으면
같은 공사장을 여러 번 방문하라고 하는 꼴이 된다.
